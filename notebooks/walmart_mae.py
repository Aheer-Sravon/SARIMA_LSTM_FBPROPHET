...
# Load the CSV data
weather_data = pd.read_csv('../data/walmart_metrices.csv')
...
fig, ax = plt.subplots(figsize=(5, 2.8))  # Single column width, height as needed
...
ax.set_title('MAE Comparison - Walmart Dataset (Store 3)', fontsize=11, fontweight='medium', pad=10)
...
ax.annotate(f'Best: {best_algorithm}\n({best_mae:.2f})',
            ...
            xytext=(best_idx, best_mae + 60000),
...
plt.savefig('walmart_mae.png', dpi=600, bbox_inches='tight')
plt.savefig('walmart_mae.pdf', bbox_inches='tight')
