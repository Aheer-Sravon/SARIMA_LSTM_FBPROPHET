...
This repository contains Python scripts and Jupyter notebooks for forecasting daily `cups_sold` using various time series models. The models include SARIMA/SARIMAX,
...
models and seasonal parameters for statistical models.

The project demonstrates model training, hyperparameter optimization (where applicable), recursive forecasting on a test set, evaluation metrics (MAE, RMSE, MAPE),
and future predictions. It is based on CSV data in the `data/` directory, with preprocessed splits (`train.csv`, `validation.csv`, `test.csv`) in `data/preprocessed/`.
...
- **CNN-LSTM**:
  ```
  cv scripts
  uv run cnn_lstm_forecasting.py
  ```
...
- **Prophet**:
  ```
  cv scripts
  uv run fbprophet_forecasting.py
  ```
